...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Enregistrer un(e) {{$categorie->nom}}</div>

                <div class="card-body">
                    <form method="POST" action="{{route('produit.add',['id'=>$categorie->id]) }}" aria-label="{{ __('Register') }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group ">
                            <label for="0" >Description</label>
                                <input id="description" type="text" class="form-control  {{ $errors->has('description') ? ' is-invalid': ' ' }}" name="description"  autofocus>
                        ...
                        <div class="form-group ">
                            <label for="contact" >Cat̩gorie</label>
                            <select name="categorie_id" class="form-control">

                            <?php $i=0; ?>

                           <?php $i++; ?>
                           <option selected value="{{$categorie->id}}">{{$i}}. {{$categorie->nom}}</option>
                        ...
                        <div class="form-group ">
                            <label for="0" >Stock Initial</label>
                                <input id="stockI" type="text" class="form-control  {{ $errors->has('stockI') ? ' is-invalid': ' ' }}" name="stockI"  autofocus>
